	...
	void declareTree(py::module& m) {
		...
		py::class_<XCTree, XCScrollView>(m, "XTree")
			...
			.def(py::init([](int x, int y, int cx, int cy, XCObjectUI* parent, int flags) {
				...
				auto eleHandle = XTree_CreateEx(x, y, cx, cy, handle, flags);
				XCTree obj;
		...
		.	def(py::init([](int cx, int cy, XCObjectUI* parent=nullptr) {
			...
			// 3.3.8 新增
			.def("setItemTemplateXMLFromMem", [](XCTree& self, const std::string& xmlData) {
				return XTree_SetItemTemplateXMLFromMem(self.getEleHandle(), xmlData.c_str());
			}, "xmlData"_a)
			.def("setItemTemplateXMLFromZipRes", [](XCTree& self, int id, const std::wstring& fileName, const std::wstring& password = L"", uintptr_t hModule = 0) {
				const wchar_t* pPassword = password.empty() ? NULL : password.c_str();
				return XTree_SetItemTemplateXMLFromZipRes(self.getEleHandle(), id, fileName.c_str(), pPassword, (HMODULE)hModule);
			}, "id"_a, "fileName"_a, "password"_a = L"", "hModule"_a = 0)
			.def("getItemTemplate", [](XCTree& self) -> XCListItemTemplate* {
				auto handle = XTree_GetItemTemplate(self.getEleHandle());
