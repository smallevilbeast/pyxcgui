	...
	void declareListBox(py::module& m) {
		...
		py::class_<XCListBox, XCScrollView>(m, "XListBox")
			...
			.def(py::init([](int x, int y, int cx, int cy, XCObjectUI* parent, int flags) {
				...
				auto eleHandle = XListBox_CreateEx(x, y, cx, cy, handle, flags);
				XCListBox obj;
		...
		.	def(py::init([](int cx, int cy, XCObjectUI* parent=nullptr) {
			...
			// 3.3.8 新增
			.def("setItemTemplateXMLFromMem", [](XCListBox& self, const std::string& xmlData) {
				return XListBox_SetItemTemplateXMLFromMem(self.getEleHandle(), xmlData.c_str());
			}, "xmlData"_a)
			.def("setItemTemplateXMLFromZipRes", [](XCListBox& self, int id, const std::wstring& fileName, const std::wstring& password = L"", uintptr_t hModule = 0) {
				const wchar_t* pPassword = password.empty() ? NULL : password.c_str();
				return XListBox_SetItemTemplateXMLFromZipRes(self.getEleHandle(), id, fileName.c_str(), pPassword, (HMODULE)hModule);
			}, "id"_a, "fileName"_a, "password"_a = L"", "hModule"_a = 0)
			.def("getItemTemplate", [](XCListBox& self) -> XCListItemTemplate* {
				auto handle = XListBox_GetItemTemplate(self.getEleHandle());
